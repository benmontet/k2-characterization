...
hosting a $2.29 \pm 0.24 R_\odot$ planet with $T_{eq} = 273 \pm 15$ K and an
...
at the rate of $\sim 1\arcsec$ hr$^{-1}$, with rapid corrections that occur
...
mission (black), with our \KT\ \Ci\ planet candidates overlaid (fuchsia).
Also included is the location of the Sun (yellow).
Nearly all of our candidates have photometry consistent with later spectral
types than G2.}
...
of stars within 1 kpc of the Sun, following the results of \citet{Hayden15}.
...
In \paperit, we present only parameters directly observable from the \KT\
light curve itself: the period, time of transit center, and transit depth.
...
For each candidate, we fit a transit and systematic model simultaneously
to the data.
The transit model is an exposure-time integrated, two-parameter limb darkened 
light curve \citep{Mandel02,Kipping10b}.
We parameterize... \dfmtodo{What are the parameters in the model?}

This method enable us to marginalize over the parameters of the
data-driven model of the instrumental systematics described in \paperit.

The results are given in Table 3.

\dfmtodo{Add more to this section.}

In this analysis, we assume the dilution caused by additional stars contributing
flux into the aperture is negligible for nearly all systems.
Given the location of the \Ci\ field at a high galactic latitude, we expect low
contamination by background giants. 
...
actually unresolved binaries, would cause us to underestimate the radii
of any planets we detect.
...
ratio between the two stars in the \kep\ bandpass, inflating the transit depth by 
this factor to account for the dilution. 
...
\caption{\btmtodo{PLACEHOLDER, UPDATE}.
...
at the Palomar 5.1-meter Hale telescope on the nights of 2015 Feb 3 and 4 UT.
...
of these measurements is adopted for our final values listed in Table~6.
...
\caption{AO data for all systems imaged.
...
For the two systems with imaged companions, EPIC 201546283 and 201828749,
only unsaturated frames are collected.
The pixel scale is 0.0252$\arcsec$ per pixel, and each subplot is a square 400 pixels
on a side. 
...
We include a list of these potential contaminants as Table 5.
...
Instead, it is at an reduced semimajor axis \ars$ = 83.2 \pm 9.1$.
Its equilibrium temperature is then, assuming zero albedo, $T_{eq} = 273 \pm 15$
K, meaning its bulk insolation is $131 \pm 30$ percent that of the Earth's.
...
They are at distances ranging from $34$ to $500$ pc, have radii ranging from $1.3$ to
$5.3 R_\odot$, and orbital periods ranging from 5.0 to 50.3 days.
Like the original mission, we find significantly more small planets than large planets.
...
A magnitude limited survey pointed in this direction would then have much in
common with a volume limited survey near the galactic plane, as massive stars
at kiloparsec distances are relatively unlikely likely to exist.
Therefore, the pool of targets that could be selected for \Ci\ contains a larger
fraction of subsolar stars.

Low-mass stars, particularly M dwarfs, are also a specific focus of the \KT\ mission.
One of the primary goals of the Kepler mission was to ``Determine the abundance of
...
\anytodo{Does anyone feel we should include anything else in the discussion?}
