...
module.
\change{We then run an MCMC \btmtodo{still a chain?} using \texttt{MULTINEST} \citep{Feroz09},
...
along that line of sight from the NED \btmtodo{acronym}, which reports the 
...
SDSS, 2MASS, and WISE calculated by \citep{Davenport14}.
In each step of our fitting process, we draw an $A_V$, calculate the expected extinction
...
The observed photometric parameters  then consistent with spectroscopically-derived
...
with derived radii in excess of 8 \rsun. For approximately one third of the others,
...
affine-invariant ensemble sampler of \citep{Goodman10}
...
On the other hand, 23 candidates
have FPP $<$ 1\%.
...
Four more of these 23 might plausibly be caused by
...
16 of which we here validate for the first time.
...
EPIC 201505350 is a two-planet system with the two planets near a 3:2 period
