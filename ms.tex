...
at the rate of $\sim 1\arcsec$ hr$^{-1}$, with rapid corrections 
...
uncertainties,\footnote{http://www.astro.princeton.edu/~tdm/k2/}
...
Typically, the instrumental false positive event appears considerably shallower than the 
...
away from nor on the same column as this candidate.
...
anomalous transit event on a different module.

Formally, our false positive validation scheme gives a FPP of $9.5 \times 10^{-4}$,
reaching our threshold for planet validation.
Since \texttt{vespa} does not account for the possibility of instrumental artifacts
in the data, this number represents only the \textit{astrophysical} false positive
probability.
